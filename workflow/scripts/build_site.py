...
def main():
    ...
    # write .nojekyll for GH Pages
    (SITE / ".nojekyll").write_text("")

    # copy the current backbone tree if present
    if TREE_SRC.exists() and TREE_SRC.stat().st_size > 0:
        shutil.copyfile(TREE_SRC, TREE_DST)
        print(f"[site] copied {TREE_SRC} -> {TREE_DST}")
    else:
        # keep an empty/placeholder if no tree yet
        TREE_DST.write_text("")
        print("[site] no backbone tree found; wrote empty latest/backbone.newick")

    # write index.html
    (SITE / "index.html").write_text(VIEWER_HTML)
    # simple stamp
    (SITE / "build.txt").write_text(dt.datetime.utcnow().isoformat() + "Z\n")
    print("[site] wrote index.html and build.txt")
